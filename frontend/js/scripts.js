...
function loadAppointmentDetails(appointmentTitle) {
    $.ajax({
        type: "GET",
        url: "../backend/serviceHandler.php",
        cache: false,
        ...
        success: function (result) {
            $("#pageContent").load("templates/appointmentList.html", function () {
                ...
                $("#pageContent").load("templates/appointmentDetails.html", function () {
                    ...
                    var appointmentCommentsAuthors = ["Harry", "Tribun"];
                    var appointmentCommentsText = ["Soo cool", "nice"];
...
function submitNewAppointmentForm() {
    ...
    for (var i = 0; i < appointmentDateOptions.length; ++i) {
        var currentDateOption = $(".appointmentDateOption")[i]; //returns html element
        var current = $(currentDateOption); //convert html element back to jqerry object
        if (current.val()) {
            found = true;
